...
namespace NetworkCommsDotNet.Connections.UDP
{
    ...
    public sealed partial class UDPConnection : IPConnection
    {
        ...
        public void SendObject<sendObjectType>(string sendingPacketType, sendObjectType objectToSend, IPEndPoint ipEndPoint)
        {
            Packet sendPacket = objectToSend as Packet;
            if (sendPacket == null)
                sendPacket = new Packet(sendingPacketType, objectToSend, NetworkComms.DefaultSendReceiveOptions);
            else
            {
            ...
            SendPacketSpecific<sendObjectType>(sendPacket, ipEndPoint);
        }
#endif
    }
}